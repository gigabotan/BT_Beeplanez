...
public class planeController : MonoBehaviour {
	...
	private float speed = 20f; // это не юзается, почему-то
	private float speeddecrease = -.005f; // уменьшение скорость
	private float turnspeed = 10f; // скорость поворота
	private Vector3 daunV = new Vector3(0f,-5f,0f); // вектор гравитации
	...
	void OnCollisionEnter2D(Collision2D coll) { // если самелетик врезался в землю
		if (!onGround) { // если уже взлетел, то true
	...
	void Update () { 
		...
		if (transform.position.y < 20f) {  // если самолетик улетел слишком высоко, то мы теряем управление
			currSpeed += Input.GetAxis ("Vertical") * Time.deltaTime; // тут прописано увеличение скорости
		}
		...
		transform.position += transform.right * currSpeed* Time.deltaTime* 50f; // тут перемещаем вамолетик вперед
